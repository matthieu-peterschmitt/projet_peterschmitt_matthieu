          ...
          @for (pollution of favoritePollutions(); track pollution.id) {
            <div class="pollution-card">
              ...
              <div class="pollution-actions">
                ...
                <button type="button" class="remove-btn" (click)="removeFavorite(pollution.id!)">
                  Retirer des favoris
                </button>
              </div>
            </div>
...
export class FavoritesComponent implements OnInit {
  ...
  private loadFavorites(): void {
    ...
    forkJoin(pollutionObservables)
      .pipe(
        catchError((error) => {
          this.errorMessage.set('Erreur lors du chargement des favoris: ' + error.message);
      ...
      .subscribe((pollutions) => {
        ...
        const validPollutions = pollutions.filter((p): p is PollutionDeclaration => p !== null);
